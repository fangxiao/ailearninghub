(function(){"use strict";const a={data:window.SITE_DATA||{categories:[],articles:[],stats:{}},activeCategory:"all",searchQuery:"",difficultyFilter:"all",viewMode:"roadmap",roadmapTrack:"tech",theme:localStorage.getItem("ai_hub_theme")||"dark",readerTheme:localStorage.getItem("ai_hub_reader_theme")||"default",readerFontSize:parseInt(localStorage.getItem("ai_hub_reader_fontsize")||"16",10),currentArticle:null,history:JSON.parse(localStorage.getItem("ai_hub_history")||"[]")},s={app:document.getElementById("app"),themeToggle:document.getElementById("theme-toggle"),themeIcon:document.getElementById("theme-icon"),searchInput:document.getElementById("search-input"),searchClear:document.getElementById("search-clear"),categoryTabs:document.getElementById("category-tabs"),categorySelect:document.getElementById("category-select"),viewButtons:document.querySelectorAll("[data-view]"),mainContent:document.getElementById("main-content"),readerModal:document.getElementById("reader-modal"),readerIframeContainer:document.getElementById("reader-iframe-container"),readerMarkdownContainer:document.getElementById("reader-markdown-container"),readerFrame:document.getElementById("reader-frame"),markdownContent:document.getElementById("markdown-content"),readerTitle:document.getElementById("reader-title"),readerCategory:document.getElementById("reader-category"),readerTocList:document.getElementById("reader-toc-list"),readerTocDrawer:document.getElementById("reader-toc-drawer"),wechatModal:document.getElementById("wechat-modal"),btnPrevArticle:document.getElementById("btn-prev-article"),btnNextArticle:document.getElementById("btn-next-article"),btnOpenExternal:document.getElementById("btn-open-external"),btnCloseReader:document.getElementById("btn-close-reader"),btnToggleToc:document.getElementById("btn-toggle-toc"),toast:document.getElementById("toast")};function $(){k(a.theme),P(),U(),z(),p(),R();const e=new URLSearchParams(window.location.search),t=e.get("category"),i=e.get("article"),n=e.get("view");if(n&&["grid","list","tree","shelf","roadmap","my-space","showcases","works"].includes(n)&&(a.viewMode=n==="works"?"showcases":n,a.viewMode=n,x()),t&&(t==="all"||a.data.categories.some(o=>o.id===t))&&(a.activeCategory=t,b()),i){const o=a.data.articles.find(d=>d.id===i||d.path===i);o&&f(o)}const r=()=>{const o=window.location.hash;o==="#showcases"||o==="#works"||o==="#tools"?(a.viewMode="showcases",x(),p()):o==="#roadmap"&&(a.viewMode="roadmap",x(),p())};window.addEventListener("hashchange",r),r()}function k(e){a.theme=e,localStorage.setItem("ai_hub_theme",e),e==="light"?(document.documentElement.classList.add("light-theme"),document.documentElement.classList.remove("dark"),s.themeIcon&&(s.themeIcon.innerHTML="\u2600\uFE0F")):(document.documentElement.classList.remove("light-theme"),document.documentElement.classList.add("dark"),s.themeIcon&&(s.themeIcon.innerHTML="\u{1F319}"))}function C(){k(a.theme==="dark"?"light":"dark")}let I;function v(e,t="\u2728"){s.toast&&(s.toast.innerHTML=`<span class="text-xl">${t}</span><span>${e}</span>`,s.toast.classList.remove("opacity-0","translate-y-4","pointer-events-none"),s.toast.classList.add("opacity-100","translate-y-0"),clearTimeout(I),I=setTimeout(()=>{s.toast.classList.add("opacity-0","translate-y-4","pointer-events-none"),s.toast.classList.remove("opacity-100","translate-y-0")},2400))}function Y(){s.statsArticles&&(s.statsArticles.textContent=a.data.articles.length),s.statsCategories&&(s.statsCategories.textContent=a.data.categories.length),s.statsEbooks&&(s.statsEbooks.textContent=a.data.categories.find(e=>e.id==="ebooks")?.count||7)}async function j(e){const t=Date.now(),n=`${t}:${e}:ai_hub_sec_982f10`;try{const o=new TextEncoder().encode(n),d=await crypto.subtle.digest("SHA-256",o),m=Array.from(new Uint8Array(d)).map(g=>g.toString(16).padStart(2,"0")).join("").substring(0,32);return{ts:String(t),sign:m}}catch{return{ts:String(t),sign:""}}}async function R(){const e=document.getElementById("visitor-id-num"),t=document.getElementById("visitor-total-num");if(!e&&!t)return;const i="ai_hub_visitor_seq_id";let n=localStorage.getItem(i);const r=!n,o=h=>Number(h).toLocaleString("en-US");n&&(n=parseInt(n,10),!isNaN(n)&&n>0&&e&&(e.textContent=o(n)));const d=r?"?action=hit":"?action=get",l=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"https://api.ailearning.top/api/visitor"+d:"/api/visitor"+d,m=await j("/api/visitor"),g={"x-request-time":m.ts,"x-request-sign":m.sign};fetch(l,{method:r?"POST":"GET",headers:g}).then(h=>h.ok?h.json():fetch("https://api.ailearning.top/api/visitor"+d,{method:r?"POST":"GET",headers:g}).then(c=>c.json())).then(h=>{if(!h||typeof h.total>"u")return;const c=parseInt(h.total,10)||1;if(r){n=c;try{localStorage.setItem(i,String(n))}catch{}}e&&(e.textContent=o(n||c)),t&&(t.textContent=o(Math.max(c,n||c)))}).catch(()=>{e&&e.textContent==="..."&&(e.textContent="1"),t&&t.textContent==="..."&&(t.textContent="1")})}function P(){if(!s.categoryTabs)return;const e=a.data.categories;let t=`
    ...
    `;e.forEach(i=>{const n=a.activeCategory===i.id;t+=`
        <button data-category="${i.id}" class="category-tab px-3.5 py-1.5 rounded-xl text-xs font-medium border flex items-center gap-1.5 transition ${n?"active":"border-white/10 text-slate-400 hover:text-white hover:bg-white/5"}">
          <span>${i.icon}</span>
          <span>${i.name}</span>
        </button>
      `}),s.categoryTabs.innerHTML=t}function U(){if(!s.categorySelect)return;let e=`<option value="all">\u{1F310} \u5168\u90E8\u7CFB\u5217\u5206\u7C7B (${a.data.articles.length} \u7BC7)</option>`;a.data.categories.forEach(t=>{e+=`<option value="${t.id}">${t.icon} ${t.name} (${t.count} \u7BC7)</option>`}),s.categorySelect.innerHTML=e,s.categorySelect.value=a.activeCategory}function b(){document.querySelectorAll(".category-tab").forEach(e=>{e.dataset.category===a.activeCategory?(e.classList.add("active"),e.classList.remove("border-white/10","text-slate-400","hover:bg-white/5")):(e.classList.remove("active"),e.classList.add("border-white/10","text-slate-400","hover:bg-white/5"))}),s.categorySelect&&(s.categorySelect.value=a.activeCategory)}function x(){s.viewButtons.forEach(e=>{e.dataset.view===a.viewMode?(e.classList.add("bg-indigo-600","text-white"),e.classList.remove("text-slate-400","hover:text-white","hover:bg-white/5")):(e.classList.remove("bg-indigo-600","text-white"),e.classList.add("text-slate-400","hover:text-white","hover:bg-white/5"))})}function A(){return a.data.articles.filter(e=>{if(a.activeCategory!=="all"&&e.categoryId!==a.activeCategory)return!1;if(a.searchQuery){const t=a.searchQuery.toLowerCase(),i=e.title.toLowerCase().includes(t),n=(e.subtitle||"").toLowerCase().includes(t),r=(e.summary||"").toLowerCase().includes(t),o=(e.toc||[]).some(l=>l.toLowerCase().includes(t)),d=(e.badge||"").toLowerCase().includes(t);if(!i&&!n&&!r&&!o&&!d)return!1}return!0})}function p(){if(!s.mainContent)return;const e=document.getElementById("category-nav-bar"),t=document.getElementById("hero-banner-section");e&&(["grid","list"].includes(a.viewMode)?e.classList.remove("hidden"):e.classList.add("hidden")),t&&(a.viewMode==="showcases"?t.classList.add("hidden"):t.classList.remove("hidden")),a.viewMode==="grid"?D():a.viewMode==="list"?N():a.viewMode==="tree"?F():a.viewMode==="shelf"?renderShelfView():a.viewMode==="roadmap"?_():a.viewMode==="my-space"?renderMySpaceView():a.viewMode==="showcases"&&G()}function u(e){if(!a.searchQuery||!e)return e;const t=a.searchQuery.replace(/[.*+?^${}()|[\]\\]/g,"\\$&"),i=new RegExp(`(${t})`,"gi");return e.replace(i,'<mark class="bg-amber-400/30 text-amber-300 px-1 rounded">$1</mark>')}function D(){const e=A(),t=a.data.categories;if(e.length===0){s.mainContent.innerHTML=L();return}if(a.activeCategory!=="all"||a.searchQuery||a.formatFilter!=="all"){const n=t.find(d=>d.id===a.activeCategory);let r="";n&&!a.searchQuery?r=O(n,e):a.searchQuery&&(r=`
      ...
      `;const o=document.getElementById("btn-clear-search-res");o&&o.addEventListener("click",()=>{a.searchQuery="",s.searchInput.value="",p()});return}let i="";t.forEach(n=>{const r=a.data.articles.filter(d=>d.categoryId===n.id);if(r.length===0)return;const o=r[0];i+=`
        <div class="mb-14" id="cat-section-${n.id}">
          <!-- Category Header -->
          <div class="p-6 md:p-8 rounded-3xl glass border border-white/10 mb-6 relative overflow-hidden">
            <div class="absolute right-0 top-0 w-64 h-64 bg-${n.theme}-500/10 blur-3xl pointer-events-none"></div>
              ...
              <div class="flex items-start md:items-center gap-4">
                ...
                <div>
                  <div class="flex items-center gap-2 flex-wrap">
                    <h2 class="text-2xl font-bold text-white">${n.name}</h2>
                    <span class="badge-chip bg-${n.theme}-500/20 text-${n.theme}-400 border border-${n.theme}-500/30">${n.badge}</span>
                    <span class="text-xs text-slate-400 bg-white/5 px-2.5 py-1 rounded-full border border-white/5">\u5171 ${r.length} \u7BC7</span>
                  </div>
                  <p class="text-sm text-slate-300 mt-2 max-w-2xl">${n.desc}</p>
                </div>
              </div>
              <div class="flex items-center gap-2.5 shrink-0">
                <button onclick="window.AI_HUB.switchCategory('${n.id}')" class="px-4 py-2 text-xs font-semibold rounded-xl bg-white/10 hover:bg-white/20 text-white transition flex items-center gap-1.5 border border-white/10">
                  <span>\u67E5\u770B\u8BE5\u7CFB\u5217\u5168\u90E8</span>
                  <span>\u2192</span>
                </button>
                ${o?`
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ${r.slice(0,6).map(T).join("")}
          </div>

          ${r.length>6?`
            <div class="text-center mt-6">
              <button onclick="window.AI_HUB.switchCategory('${n.id}')" class="px-6 py-2.5 rounded-2xl glass text-xs font-medium text-slate-300 hover:text-white hover:border-indigo-500/50 transition">
                \u67E5\u770B\u8BE5\u7CFB\u5217\u5269\u4F59 ${r.length-6} \u7BC7\u6587\u6863 \u2193
              </button>
            </div>
          `:""}
        </div>
      `}),s.mainContent.innerHTML=i}function O(e,t){return`
    ...
    `}function T(e){const t=a.data.categories.find(n=>n.id===e.categoryId)||{};let i="bg-blue-500/20 text-blue-400 border-blue-500/30";return e.format==="pdf"&&(i="bg-rose-500/20 text-rose-400 border-rose-500/30"),e.format==="md"&&(i="bg-amber-500/20 text-amber-400 border-amber-500/30"),`
      <div class="article-card rounded-2xl glass border border-white/10 p-6 flex flex-col justify-between relative group">
        <div>
          ...
          <p class="text-xs text-slate-400 mt-2.5 line-clamp-3 leading-relaxed">
            ${u(e.summary||"\u70B9\u51FB\u8FDB\u5165\u9605\u8BFB\u5168\u7BC7\u8BE6\u7EC6\u56FE\u6587\u5185\u5BB9\u4E0E\u6280\u672F\u62C6\u89E3\u3002")}
          </p>
        </div>
    ...
    `}function N(){const e=A();if(e.length===0){s.mainContent.innerHTML=L();return}s.mainContent.innerHTML=`
      <div class="rounded-2xl sm:rounded-3xl glass border border-white/10 overflow-hidden animate-fade-in">
        ...
        <div class="block sm:hidden divide-y divide-white/5">
          ${e.map(t=>{const i=a.data.categories.find(n=>n.id===t.categoryId)||{};return`
              <div class="p-4 hover:bg-white/5 transition flex flex-col gap-2.5">
                ...
                ${t.subtitle?`<div class="text-xs text-slate-400 line-clamp-1">${u(t.subtitle)}</div>`:""}
              ...
              ${e.map(t=>{const i=a.data.categories.find(n=>n.id===t.categoryId)||{};return`
                  <tr class="hover:bg-white/5 transition">
                    ...
                    <td class="px-6 py-4">
                      ...
                      ${t.subtitle?`<div class="text-xs text-slate-400 mt-0.5 line-clamp-1">${u(t.subtitle)}</div>`:""}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
    ...
    `}function F(){const e=a.data.categories;s.mainContent.innerHTML=`
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 animate-fade-in">
        <div class="lg:col-span-1 space-y-3">
          <div class="p-4 sm:p-6 rounded-2xl sm:rounded-3xl glass border border-white/10">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-1 gap-1.5">
              ${e.map(t=>{const i=a.data.articles.filter(n=>n.categoryId===t.id).length;return`
        ...
        <div class="lg:col-span-2 space-y-6">
          ${e.map(t=>{const i=a.data.articles.filter(n=>n.categoryId===t.id);return i.length===0?"":`
              <div id="tree-cat-${t.id}" class="p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl glass border border-white/10">
                ...
                <div class="space-y-2.5 sm:space-y-3">
                  ${i.map((n,r)=>`
                      <div class="p-3 sm:p-3.5 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/10 transition flex items-center justify-between gap-3 sm:gap-4 cursor-pointer group" onclick="window.AI_HUB.openReaderById('${n.id}')">
                        <div class="flex items-center gap-2.5 sm:gap-3 min-w-0">
                          ...
                          <div class="min-w-0">
                            ...
                            ${n.subtitle?`<div class="text-[11px] sm:text-xs text-slate-400 truncate mt-0.5">${n.subtitle}</div>`:""}
                          </div>
                        </div>

                        <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                          <span class="text-[10px] sm:text-xs text-slate-400">${n.readTime}</span>
                          <span class="badge-chip text-[10px] sm:text-xs bg-white/5 border border-white/10 text-slate-300 hidden sm:inline-block">${n.badge}</span>
                        </div>
                      </div>
                    `).join("")}
                </div>
    ...
    `}function _(){const e=a.roadmapTrack==="tech",n=e?[{level:"Level 1",title:"\u{1F331} \u57FA\u7840\u901A\u8BC6\u4E0E\u5927\u6A21\u578B\u5E95\u5C42\u539F\u7406",desc:"\u6DF1\u5165\u638C\u63E1\u5927\u6A21\u578B\u6838\u5FC3\u67B6\u6784\u3001\u6CE8\u610F\u529B\u673A\u5236 (Self-Attention)\u3001RoPE \u4E0E\u4F4D\u7F6E\u7F16\u7801\u6570\u5B66\u539F\u7406\u4E0E\u524D\u6CBF\u8109\u7EDC",categories:["ai-science","ai-news","bigmodel","ai-safety"],skills:["Transformer \u67B6\u6784","\u6CE8\u610F\u529B\u673A\u5236\u8BA1\u7B97","Token & \u5D4C\u5165\u5411\u91CF","\u6A21\u578B\u5B89\u5168\u4E0E\u4F26\u7406"]},{level:"Level 2",title:"\u26A1 Agent \u6838\u5FC3\u67B6\u6784\u3001\u624B\u5199\u6846\u67B6\u4E0E\u601D\u8003\u5FAA\u73AF",desc:"\u4ECE\u96F6\u624B\u5199\u8F7B\u91CF\u7EA7 Agent \u601D\u8003\u5FAA\u73AF\uFF0C\u6DF1\u5165 OpenClaw \u667A\u80FD\u4F53\u7CFB\u7EDF\uFF0C\u638C\u63E1\u6709\u9650\u72B6\u6001\u673A\u3001\u610F\u56FE\u611F\u77E5\u4E0E\u8BB0\u5FC6\u7CFB\u7EDF",categories:["myagent","agent-core","openclaw"],skills:["ReAct \u601D\u8003\u5FAA\u73AF","\u77ED/\u957F\u671F\u8BB0\u5FC6\u7CFB\u7EDF","\u5DE5\u5177\u8C03\u7528\u534F\u8BAE","OpenClaw \u667A\u80FD\u4F53"]},{level:"Level 3",title:"\u{1F6E0}\uFE0F \u73B0\u4EE3 AI \u5DE5\u5177\u94FE\u4E0E AI Coding \u5168\u6808\u8FDB\u9636",desc:"\u73A9\u8F6C Claude Code \u5168\u5BB6\u6876\u3001AI Coding 33\u7BC7\u5168\u4F53\u7CFB\u3001MCP Server \u534F\u8BAE\u5F00\u53D1\u3001TDD \u4E0E\u540E\u7AEF\u670D\u52A1\u96C6\u6210",categories:["aitools","ai-coding"],skills:["Claude Code \u5168\u5BB6\u6876","MCP \u534F\u8BAE\u5F00\u53D1","AI \u8F85\u52A9\u67B6\u6784\u8BBE\u8BA1","\u6D4B\u8BD5\u9A71\u52A8\u5F00\u53D1 TDD"]},{level:"Level 4",title:"\u{1F4BC} \u4F01\u4E1A\u7EA7\u5DE5\u7A0B\u843D\u5730\u4E0E\u79C1\u6709\u77E5\u8BC6\u5E93 (RAG)",desc:"\u843D\u5730\u4F01\u4E1A\u667A\u80FD\u5BA2\u670D\u5168\u95ED\u73AF\u3001\u6784\u5EFA\u79C1\u6709\u672C\u5730\u5927\u6A21\u578B\u77E5\u8BC6\u5E93 (Ollama + \u5411\u91CF\u5E93)\u3001\u6253\u9020\u5782\u76F4\u9886\u57DF Agent \u7CFB\u7EDF",categories:["customer-service","customer-service-harness","local-rag","english-agent","quant-agent"],skills:["\u4F01\u4E1A\u5FAE\u4FE1\u667A\u80FD\u5BA2\u670D","\u672C\u5730\u79C1\u6709 RAG","\u5411\u91CF\u68C0\u7D22\u4E0E\u91CD\u6392","\u5782\u76F4 Agent \u843D\u5730"]}]:[{level:"Level 1",title:"\u{1F3AF} \u4E1A\u52A1\u8BA4\u77E5\u4E0E\u9AD8\u9636\u63D0\u793A\u5DE5\u7A0B (Prompt \u8D4B\u80FD\u4E1A\u52A1)",desc:"\u628A\u4E1A\u52A1\u89C4\u5219\u8F6C\u5316\u4E3A\u5927\u6A21\u578B\u80FD\u7CBE\u51C6\u6267\u884C\u7684\u9AD8\u9636 Prompt\uFF0C\u638C\u63E1\u7ED3\u6784\u5316\u8F93\u51FA\u3001\u4E1A\u52A1\u8FB9\u754C\u7EA6\u675F\u4E0E\u5185\u5BB9\u6DA6\u8272",skills:["CRISPE \u7ED3\u6784\u5316\u6846\u67B6","\u4E1A\u52A1\u89C4\u5219\u4E0E\u8FB9\u754C\u7EA6\u675F","\u975E\u7ED3\u6784\u5316\u6570\u636E\u521D\u7B5B","\u81EA\u5A92\u4F53\u56FE\u6587\u751F\u4EA7\u6D41","\u6587\u672C\u53BB AI \u75D5\u6DA6\u8272"],curatedArticles:[{id:"aitools_pm_skills_write_prd_\u6DF1\u5EA6\u5B9E\u6218_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F3AF} \u4EA7\u54C1PRD\u5B9E\u6218"},{id:"aitools_baoyu_skills_\u4E2D\u6587\u81EA\u5A92\u4F53\u6280\u80FD\u96C6_\u516C\u4F17\u53F7\u7248_html",tag:"\u26A1 \u56FE\u6587\u751F\u4EA7\u6D41"},{id:"aitools_humanizer_skill\u4ECB\u7ECD_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F4DD} \u6587\u672C\u53BBAI\u75D5"}],inProgressText:"\u{1F331} \u300A\u4E1A\u52A1\u573A\u666F\u9AD8\u9636 Prompt \u7ED3\u6784\u5316\u8BBE\u8BA1\u5168\u6307\u5357\u300B\u5BA1\u6838\u6392\u671F\u4E2D..."},{level:"Level 2",title:"\u{1F4A1} AI \u4EA7\u54C1\u8BBE\u8BA1\u4E0E\u4E1A\u52A1\u6D41\u91CD\u6784 (AI PM \u5B9E\u6218)",desc:"\u638C\u63E1 AI \u539F\u751F\u4EA7\u54C1\u9700\u6C42\u5B9A\u4E49\uFF08AI PRD \u89C4\u8303\uFF09\u3001\u4EBA\u673A\u534F\u540C\u4EA4\u4E92\u6A21\u5F0F\u8BBE\u8BA1\u3001\u5168\u7F51\u7814\u62A5\u8C03\u7814\u4E0E\u4E1A\u52A1\u6D41\u7A0B\u81EA\u52A8\u5316",skills:["AI PRD \u7ED3\u6784\u5316\u64B0\u5199","Copilot vs Agent \u9009\u578B","\u5168\u7F51\u7ADE\u54C1\u6DF1\u5EA6\u8C03\u7814","\u4E1A\u52A1\u6D41\u7A0B\u81EA\u52A8\u5316"],curatedArticles:[{id:"aitools_pm_skills_\u4EA7\u54C1\u7ECF\u7406\u6280\u80FD\u5E02\u573A_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F3AF} PM \u6280\u80FD\u5E02\u573A"},{id:"aitools_agent_reach_\u8BA9AI\u4E0A\u7F51\u641C\u8D44\u6599_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F50D} \u5168\u7F51\u7814\u62A5\u641C\u96C6"}],inProgressText:"\u{1F331} \u300AAI \u539F\u751F PRD \u9700\u6C42\u89C4\u8303\u4E0E\u4EBA\u673A\u534F\u540C\u4EA4\u4E92\u8BBE\u8BA1\u300B\u5BA1\u6838\u6392\u671F\u4E2D..."},{level:"Level 3",title:"\u{1F916} \u96F6\u4EE3\u7801\u642D\u5EFA\u4E1A\u52A1\u667A\u80FD\u4F53 (Dify / \u6263\u5B50\u5B9E\u64CD)",desc:"\u65E0\u9700\u7F16\u5199\u590D\u6742\u4EE3\u7801\uFF0C\u901A\u8FC7\u53EF\u89C6\u5316\u754C\u9762\u642D\u5EFA\u4F01\u4E1A\u4E13\u5C5E\u77E5\u8BC6\u95EE\u7B54\u52A9\u624B\u3001\u4E1A\u52A1\u81EA\u52A8\u5316\u5DE5\u4F5C\u6D41\u4E0E\u667A\u80FD\u5BA2\u670D\u673A\u5668\u4EBA",skills:["\u4F01\u4E1A\u79C1\u6709\u77E5\u8BC6\u95EE\u7B54\u5E93","\u53EF\u89C6\u5316\u95EE\u7B54\u4EA4\u4E92\u754C\u9762","\u4E1A\u52A1\u75DB\u70B9\u4E0E\u610F\u56FE\u5206\u6790","\u5FAE\u4FE1/\u4F01\u5FAE\u5BA2\u670D\u63A5\u5165"],curatedArticles:[{id:"local_llm_knowledge_base_\u672C\u5730\u5927\u6A21\u578B\u77E5\u8BC6\u5E93\u7CFB\u5217_01_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F4DA} \u79C1\u6709\u77E5\u8BC6\u5E93"},{id:"local_llm_knowledge_base_\u672C\u5730\u5927\u6A21\u578B\u77E5\u8BC6\u5E93\u7CFB\u5217_04_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F5A5}\uFE0F \u53EF\u89C6\u5316\u95EE\u7B54"},{id:"customer_service_agent_\u5BA2\u670DAgent\u5B9E\u6218\u7CFB\u5217_01_html",tag:"\u{1F4AC} \u4E1A\u52A1\u75DB\u70B9\u5206\u6790"}],inProgressText:"\u{1F331} \u300A\u96F6\u4EE3\u7801\u5E73\u53F0\u6DF1\u5EA6\u6A2A\u8BC4\u4E0E\u4F01\u4E1A\u79C1\u6709\u77E5\u8BC6\u5E93 Bot \u5B9E\u64CD\u300B\u5BA1\u6838\u6392\u671F\u4E2D..."},{level:"Level 4",title:"\u{1F4C8} \u4E1A\u52A1 ROI \u8BC4\u4F30\u4E0E\u5546\u4E1A\u5316\u95ED\u73AF (\u51B3\u7B56\u4E0E\u843D\u5730)",desc:"\u638C\u63E1 AI \u9879\u76EE\u6295\u5165\u4EA7\u51FA\u6BD4\uFF08ROI\uFF09\u6D4B\u7B97\u3001\u4E1A\u52A1\u56E2\u961F\u5F15\u5165 SOP\u3001\u4E00\u4EBA\u516C\u53F8\uFF08Solopreneur\uFF09\u95ED\u73AF\u4E0E\u4F01\u4E1A\u843D\u5730\u6307\u5357",skills:["\u4F01\u4E1A AI \u9009\u578B ROI \u8D26\u672C","\u4E1A\u52A1\u56E2\u961F\u5F15\u5165 SOP","\u4E00\u4EBA\u516C\u53F8\u5546\u4E1A\u95ED\u73AF","\u590D\u6742\u4E1A\u52A1\u591A\u4F53\u534F\u540C"],curatedArticles:[{id:"customer_service_agent_\u5BA2\u670DAgent\u5B9E\u6218\u7CFB\u5217_08_\u516C\u4F17\u53F7\u7248_html",tag:"\u{1F91D} \u590D\u6742\u4E1A\u52A1\u4F1A\u8BCA"},{id:"ai_coding_AI_Coding\u7CFB\u5217_25_\u56E2\u961F\u534F\u4F5C_html",tag:"\u{1F465} \u56E2\u961F\u534F\u540C\u843D\u5730"}],inProgressText:"\u{1F331} \u300A\u4F01\u4E1A\u5F15\u5165 AI \u7684 ROI \u6210\u672C\u8D26\u672C\u4E0E\u4E1A\u52A1\u56E2\u961F\u843D\u5730 SOP\u300B\u5BA1\u6838\u6392\u671F\u4E2D..."}];s.mainContent.innerHTML=`
      <div class="max-w-4xl mx-auto animate-fade-in px-1 sm:px-0">
        ...
        <div class="space-y-6 sm:space-y-8">
          ${n.map(r=>{const o=e?a.data.articles.filter(d=>r.categories.includes(d.categoryId)).length:(r.curatedArticles||[]).length;return`
              <div class="roadmap-step">
                ...
                <div class="p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl glass border border-white/10">
                  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4 border-b border-white/10 pb-3 sm:pb-4 mb-3 sm:mb-4">
                    <div>
                      <span class="text-[11px] sm:text-xs font-mono font-bold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md border border-indigo-500/20">${r.level}</span>
                      <h3 class="text-lg sm:text-xl font-bold text-white mt-1.5">${r.title}</h3>
                  ...
                  ${e?`
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 mt-3 sm:mt-4">
                      ${r.categories.map(d=>{const l=a.data.categories.find(g=>g.id===d);if(!l)return"";const m=a.data.articles.filter(g=>g.categoryId===d).length;return`
                  ...
                  `:`
                    ...
                    <div class="space-y-3 mt-3 sm:mt-4">
                      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                        ${(r.curatedArticles||[]).map(d=>{const l=a.data.articles.find(m=>m.id===d.id);return l?`
    ...
    `}function V(e){return e?e.replace(/^### (.*$)/gim,'<h3 id="$1">$1</h3>').replace(/^## (.*$)/gim,'<h2 id="$1">$1</h2>').replace(/^# (.*$)/gim,'<h1 id="$1">$1</h1>').replace(/^\> (.*$)/gim,"<blockquote>$1</blockquote>").replace(/\*\*(.*)\*\*/gim,"<strong>$1</strong>").replace(/\*(.*)\*/gim,"<em>$1</em>").replace(/```([a-z]*)\n([\s\S]*?)```/gim,"<pre><code>$2</code></pre>").replace(/`([^`]+)`/gim,"<code>$1</code>").replace(/\n\n/gim,"</p><p>").replace(/\[([^\]]+)\]\(([^)]+)\)/gim,'<a href="$2" target="_blank">$1</a>'):""}function f(e){a.currentArticle=e,a.history=[{id:e.id,time:new Date().toLocaleTimeString()},...a.history.filter(i=>i.id!==e.id)],localStorage.setItem("ai_hub_history",JSON.stringify(a.history)),window.history.replaceState(null,"",`?article=${e.id}`),s.readerTitle&&(s.readerTitle.textContent=e.title);const t=a.data.categories.find(i=>i.id===e.categoryId);s.readerCategory&&(s.readerCategory.textContent=t?`${t.icon} ${t.name}`:"\u7CFB\u5217\u6587\u6863"),W(),e.format==="md"?(s.readerIframeContainer&&s.readerIframeContainer.classList.add("hidden"),s.readerMarkdownContainer&&s.readerMarkdownContainer.classList.remove("hidden"),s.markdownContent&&(s.markdownContent.innerHTML='<div class="text-center py-16 text-slate-400">\u6B63\u5728\u89E3\u6790\u5E76\u6E32\u67D3 Markdown \u6587\u6863...</div>'),fetch(e.path).then(i=>i.text()).then(i=>{let n="";window.marked&&typeof window.marked.parse=="function"?n=window.marked.parse(i):n=V(i),s.markdownContent&&(s.markdownContent.innerHTML=n);const r=[];i.split(`
`).forEach(d=>{(d.startsWith("## ")||d.startsWith("### "))&&r.push(d.replace(/^#+\s*/,"").trim())}),e.toc=r.slice(0,15),B(e)}).catch(i=>{s.markdownContent&&(s.markdownContent.innerHTML=`<div class="text-rose-400 p-6">\u52A0\u8F7D Markdown \u6587\u4EF6\u5931\u8D25\uFF1A${i.message}</div>`)})):(s.readerMarkdownContainer&&s.readerMarkdownContainer.classList.add("hidden"),s.readerIframeContainer&&s.readerIframeContainer.classList.remove("hidden"),s.readerFrame&&(s.readerFrame.src=e.path,s.readerFrame.onload=function(){try{const i=s.readerFrame.contentDocument||s.readerFrame.contentWindow.document;if(!i)return;const n=i.createElement("style");n.textContent=`
              body {
            ...
            `,i.head.appendChild(n),i.querySelectorAll("a[href]").forEach(o=>{o.addEventListener("click",function(d){const l=o.getAttribute("href");if(!l)return;if(l.startsWith("#")){d.preventDefault();const c=i.querySelector(l);c&&c.scrollIntoView({behavior:"smooth"});return}if(l.startsWith("http://")||l.startsWith("https://")||l.startsWith("//")){o.target="_blank";return}d.preventDefault();const m=l.split("?")[0].split("#")[0],g=m.split("/").pop(),h=a.data.articles.find(c=>c.path===m||c.path.endsWith(m)||c.path.endsWith(g)||c.id===m.replace(/\.html$/,""));if(h)f(h);else{let c=e.path.substring(0,e.path.lastIndexOf("/")+1),H=new URL(l,window.location.origin+"/"+c).pathname.replace(/^\//,"");s.readerFrame.src=H}})})}catch(i){console.warn("Iframe link handling notice:",i)}}),B(e)),s.readerModal&&(s.readerModal.classList.remove("hidden"),document.body.style.overflow="hidden")}function M(){a.currentArticle=null,s.readerModal&&(s.readerModal.classList.add("hidden"),document.body.style.overflow=""),s.readerFrame&&(s.readerFrame.src="about:blank"),window.history.replaceState(null,"",window.location.pathname),p()}function W(){if(!a.currentArticle)return;const e=a.currentArticle;s.btnOpenExternal&&(s.btnOpenExternal.href=e.path);const t=document.getElementById("btn-open-wechat-version");if(t)if(e.path&&(e.path.includes("business-review/")||e.path.includes("-\u516C\u4F17\u53F7\u7248.html"))){const r=e.path.includes("-\u516C\u4F17\u53F7\u7248.html")?e.path:e.path.replace(/\.html$/,"-\u516C\u4F17\u53F7\u7248.html");t.href=r,t.classList.remove("hidden")}else t.classList.add("hidden");const i=a.data.articles.filter(r=>r.categoryId===e.categoryId),n=i.findIndex(r=>r.id===e.id);s.btnPrevArticle&&(s.btnPrevArticle.disabled=n<=0,s.btnPrevArticle.classList.toggle("opacity-30",n<=0),s.btnPrevArticle.classList.toggle("cursor-not-allowed",n<=0)),s.btnNextArticle&&(s.btnNextArticle.disabled=n===-1||n>=i.length-1,s.btnNextArticle.classList.toggle("opacity-30",n===-1||n>=i.length-1),s.btnNextArticle.classList.toggle("cursor-not-allowed",n===-1||n>=i.length-1))}function B(e){if(!s.readerTocList)return;const t=e.toc||[];if(t.length===0){s.readerTocList.innerHTML='<div class="text-xs text-slate-400 py-4 text-center">\u672C\u7BC7\u65E0\u591A\u7EA7\u6807\u9898\u7D22\u5F15</div>';return}s.readerTocList.innerHTML=t.map((i,n)=>`
    ...
    `).join("")}function q(e){if(a.currentArticle&&a.currentArticle.format==="md"&&s.markdownContent){const t=s.markdownContent.querySelectorAll("h1, h2, h3, h4");for(const i of t)if(i.textContent.trim().includes(e)||e.includes(i.textContent.trim())){i.scrollIntoView({behavior:"smooth",block:"start"});break}}}function w(e){if(!a.currentArticle)return;const t=a.data.articles.filter(n=>n.categoryId===a.currentArticle.categoryId),i=t.findIndex(n=>n.id===a.currentArticle.id);e==="prev"&&i>0?f(t[i-1]):e==="next"&&i<t.length-1&&f(t[i+1])}function E(){s.wechatModal&&(s.wechatModal.classList.remove("hidden"),document.body.style.overflow="hidden")}function S(){s.wechatModal&&(s.wechatModal.classList.add("hidden"),document.body.style.overflow="")}function Q(){const e="\u5927\u524D\u7AEF\u5DE5\u7A0B\u5E08";navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).then(()=>{v("\u5DF2\u590D\u5236\u516C\u4F17\u53F7\u540D\u79F0\uFF1A\u5927\u524D\u7AEF\u5DE5\u7A0B\u5E08","\u{1F4CB}")}).catch(()=>{y(e)}):y(e)}function J(e){navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).then(()=>{v(`\u5DF2\u590D\u5236\u53E3\u4EE4\u3010${e}\u3011\uFF0C\u8BF7\u5728\u5FAE\u4FE1\u516C\u4F17\u53F7\u540E\u53F0\u76F4\u63A5\u53D1\u9001\uFF01`,"\u{1F4CB}")}).catch(()=>{y(e)}):y(e),E()}function y(e){const t=document.createElement("input");t.value=e,document.body.appendChild(t),t.select();try{document.execCommand("copy"),v("\u5DF2\u590D\u5236\u516C\u4F17\u53F7\u540D\u79F0\uFF1A\u5927\u524D\u7AEF\u5DE5\u7A0B\u5E08","\u{1F4CB}")}catch{v("\u516C\u4F17\u53F7\u540D\u79F0\uFF1A\u5927\u524D\u7AEF\u5DE5\u7A0B\u5E08","\u{1F4AC}")}document.body.removeChild(t)}function z(){s.themeToggle&&s.themeToggle.addEventListener("click",C),s.searchInput&&s.searchInput.addEventListener("input",t=>{a.searchQuery=t.target.value.trim(),s.searchClear&&s.searchClear.classList.toggle("hidden",!a.searchQuery),a.searchQuery&&a.viewMode!=="grid"&&a.viewMode!=="list"&&(a.viewMode="grid",x()),p()}),s.searchClear&&s.searchClear.addEventListener("click",()=>{a.searchQuery="",s.searchInput.value="",s.searchClear.classList.add("hidden"),s.searchInput.focus(),p()}),document.querySelectorAll(".hot-tag-btn").forEach(t=>{t.addEventListener("click",()=>{const i=t.dataset.keyword;i&&s.searchInput&&(s.searchInput.value=i,a.searchQuery=i,s.searchClear&&s.searchClear.classList.remove("hidden"),a.viewMode!=="grid"&&a.viewMode!=="list"&&(a.viewMode="grid",x()),p(),s.mainContent?.scrollIntoView({behavior:"smooth",block:"start"}))})});function e(){if(!s.mainContent)return;if(a.viewMode==="showcases"){window.scrollTo({top:0,behavior:"smooth"});return}const t=document.getElementById("category-nav-bar");if(t&&!t.classList.contains("hidden")){const i=t.getBoundingClientRect(),n=window.pageYOffset+i.top-65;window.scrollTo({top:Math.max(0,n),behavior:"smooth"})}else{const i=s.mainContent.getBoundingClientRect(),n=window.pageYOffset+i.top-70;window.scrollTo({top:Math.max(0,n),behavior:"smooth"})}}s.categoryTabs&&s.categoryTabs.addEventListener("click",t=>{const i=t.target.closest("[data-category]");i&&(a.activeCategory=i.dataset.category,a.activeCategory!=="all"&&a.viewMode==="roadmap"&&(a.viewMode="grid",x()),b(),p(),e())}),s.categorySelect&&s.categorySelect.addEventListener("change",t=>{a.activeCategory=t.target.value,a.activeCategory!=="all"&&a.viewMode==="roadmap"&&(a.viewMode="grid",x()),b(),p(),e()}),s.viewButtons.forEach(t=>{t.addEventListener("click",()=>{a.viewMode=t.dataset.view,x(),p(),e()})}),s.btnCloseReader&&s.btnCloseReader.addEventListener("click",M),s.btnPrevArticle&&s.btnPrevArticle.addEventListener("click",()=>w("prev")),s.btnNextArticle&&s.btnNextArticle.addEventListener("click",()=>w("next")),s.btnToggleToc&&s.btnToggleToc.addEventListener("click",()=>{s.readerTocDrawer&&s.readerTocDrawer.classList.toggle("hidden")}),document.addEventListener("keydown",t=>{(t.metaKey||t.ctrlKey)&&(t.key==="k"||t.key==="K")&&(t.preventDefault(),s.searchInput?.focus(),s.searchInput?.select()),t.key==="Escape"&&(a.currentArticle&&M(),s.wechatModal&&!s.wechatModal.classList.contains("hidden")&&S()),a.currentArticle&&!t.target.matches("input, textarea")&&(t.key==="ArrowLeft"&&w("prev"),t.key==="ArrowRight"&&w("next")),(t.key==="t"||t.key==="T")&&!t.target.matches("input, textarea")&&!a.currentArticle&&C()})}function G(){let t=`
      <div class="mb-10 animate-fade-in pt-4 sm:pt-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
          ${(a.data.showcases||[]).map(K).join("")}
        </div>

      </div>
    `;s.mainContent.innerHTML=t}function K(e){const t=e.theme||"emerald",i=(e.features||[]).map(o=>`
    ...
    `}window.AI_HUB={openReaderById:function(e){const t=a.data.articles.find(i=>i.id===e);t&&f(t)},openReaderByPath:function(e){if(!e)return;const t=e.split("#")[0],i=a.data.articles.find(n=>n.path===t||n.path.endsWith(t)||t.endsWith(n.path));i&&f(i)},switchCategory:function(e){a.activeCategory=e,a.viewMode="grid",b(),x(),p(),scrollToMainContent()},openWeChatModal:E,closeWeChatModal:S,copyWeChatName:Q,scrollToHeading:q,resetFilters:function(){a.activeCategory="all",a.searchQuery="",s.searchInput&&(s.searchInput.value=""),b(),p()},switchView:function(e){a.viewMode=e,x(),p(),scrollToMainContent()},switchRoadmapTrack:function(e){a.roadmapTrack=e,_()}},document.readyState==="loading"?document.addEventListener("DOMContentLoaded",$):$()})();